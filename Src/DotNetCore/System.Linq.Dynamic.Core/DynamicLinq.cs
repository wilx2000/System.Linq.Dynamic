...
namespace System.Linq.Dynamic
{
    ...
    internal class ExpressionParser
    {
        ...
        static readonly Type[] predefinedTypes = {
            ...
            typeof(Math),
            typeof(Convert)
            // TODO: Implement EntityFunctions
            //,
			//typeof(System.Data.Entity.Core.EntityFunctions)
...
#pragma warning restore 0219
        ...
        MemberInfo FindPropertyOrField(Type type, string memberName, bool staticAccess)
        {
            ...
            foreach (Type t in SelfAndBaseTypes(type))
            {
                MemberInfo[] members = t.GetTypeInfo().FindMembers(MemberTypes.Property | MemberTypes.Field,
                    flags, (MemberInfo m, object filterCriteria) => { return m.Name.Equals(filterCriteria.ToString(), StringComparison.OrdinalIgnoreCase); }, memberName);
        ...
        int FindMethod(Type type, string methodName, bool staticAccess, Expression[] args, out MethodBase method)
        {
            ...
            foreach (Type t in SelfAndBaseTypes(type))
            {
                MemberInfo[] members = t.GetTypeInfo().FindMembers(MemberTypes.Method,
                    flags, (MemberInfo m, object filterCriteria) => { return m.Name.Equals(filterCriteria.ToString(), StringComparison.OrdinalIgnoreCase); }, methodName);
        ...
        static object ParseEnum(string name, Type type)
        {
            if (type.GetTypeInfo().IsEnum)
            {
                MemberInfo[] memberInfos = type.GetTypeInfo().FindMembers(MemberTypes.Field,
                    ...
                    (MemberInfo m, object filterCriteria) => { return m.Name.Equals(filterCriteria.ToString(), StringComparison.OrdinalIgnoreCase); }, name);
        ...
        static bool IsCompatibleWith(Type source, Type target)
        {
            ...
            if (st.GetTypeInfo().IsEnum & !tt.GetTypeInfo().IsEnum) // If the source is an enum and the target is numeric 
            {
                switch (tc)
                {
                    case TypeCode.Int16:
